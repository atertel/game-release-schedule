    ...
    if(status !== "success") {
        ...
        page.includeJs("//ajax.googleapis.com/ajax/libs/jquery/1.6.1/jquery.min.js", function() {
            ...
            window.setTimeout(function() {
                buildList(page.content, 121);
                buildList(page.content, 120);
                buildList(page.content, 124);
                buildList(page.content, 0);
                phantom.exit();
            }, 2500);
        });
    }
...
function buildList(dom, plat) {
    ...
    //updates page for next platform
    page.evaluate(function(platNum) {
        $('#platform').val(platNum);
        document.querySelector('#platform').dispatchEvent(new Event('change', { 'bubbles': true}));
        $('#search_advanced_form').submit();
    }, platNum);
    window.setTimeout(function() {
